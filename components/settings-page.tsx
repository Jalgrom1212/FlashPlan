...
import { useCreatedPlans } from "@/lib/hooks/use-created-plans"
...
export function SettingsPage() {
  ...
  // fetch plans the logged in user has created
  const { plans: createdPlans, isLoading: createdLoading } = useCreatedPlans()
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        ...
        {user && (
          <div>
            <h2 className="text-lg font-bold text-foreground mb-4">Tus planes creados</h2>
            {createdLoading ? (
              <p className="text-sm text-muted-foreground">Cargando...</p>
            ) : createdPlans.length === 0 ? (
              <p className="text-sm text-muted-foreground">No has creado ningún plan aún.</p>
            ) : (
              <div className="space-y-3">
                {createdPlans.map((plan: any) => (
                  <Card key={plan.id} className="p-4 bg-card border-border flex justify-between items-center">
                    <div className="min-w-0">
                      <h3 className="font-semibold text-foreground line-clamp-1">{plan.name}</h3>
                      <p className="text-xs text-muted-foreground">{plan.category} • {plan.date} {plan.time}</p>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
