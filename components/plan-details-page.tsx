...
export function PlanDetailsPage({ planId }: PlanDetailsProps) {
  ...
  const handleOpenMaps = () => {
    let mapsUrl = ""
    if (planDetails && planDetails.latitude && planDetails.longitude) {
      mapsUrl = `https://www.google.com/maps/search/?api=1&query=${planDetails.latitude},${planDetails.longitude}`
    } else if (planDetails && planDetails.address) {
      mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(planDetails.address)}`
    } else {
      mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(planDetails.location)}`
    }
